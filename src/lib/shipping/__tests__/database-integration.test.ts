...
describe('Database Integration Tests', () => {
  ...
  describe('Real Data Validation', () => {
    ...
    test('should find size tiers for medium package (25x25x10)', async () => {
      ...
      const rakurakuTier60 = tier60Options.find(opt => 
        opt.categoryName === 'らくらくメルカリ便'
      );
      const yuyuTier60 = tier60Options.find(opt => 
        opt.categoryName === 'ゆうゆうメルカリ便'
      );
      ...
      if (result.totalAvailable > 0) {
        result.groups.forEach(group => {
          group.options.forEach(option => {
            console.log(`Found: ${option.optionName} - constraints: maxSize=${option.maxSizeCm}, maxLength=${option.maxLengthCm}`);
          });
        });
      }
  ...
  describe('Weight Warnings Accuracy', () => {
    test('should generate accurate weight warnings from real data', async () => {
      ...
      const hasNekoposWarning = result.weightWarnings?.some(w => 
        w.includes('ネコポス') && w.includes('1kg')
      );
      const hasYuPacketWarning = result.weightWarnings?.some(w => 
        w.includes('ゆうパケット') && w.includes('1kg')
      );
